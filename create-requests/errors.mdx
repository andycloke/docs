...
You have a [linked field in the response field mapping](
/create-requests/response-field-mapping#linked-records-in-response-field-mapping) and Data Fetcher needs to create records in the linked table (in order to link to them) but is not able to. This typically happens for two different reasons:
...
Your request uses a variable but this variable no longer exists in your Data Fetcher workspace.  You need to [create the variable](/create-requests/use-variables-in-requests#create-a-variable) again in your workspace or update the request to not use this variable.
