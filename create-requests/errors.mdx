...
Data Fetcher only searches for existing records in your selected destination view, not the entire table. If filters hide existing records, Data Fetcher can't find them and creates duplicates.

**Solution:** Create a dedicated view for Data Fetcher with no filters. Also verify your [**Match records using**](/create-requests/write-and-merge-data#choose-a-field-to-match-on) setting is using a unique identifier field.
...
## **Destination table "X" not found in base**

The destination table you selected has been deleted. You need to select a different destination table for this request.
...
The destination view you selected has been deleted. You need to select a different destination view for this request.
...
Your Airtable OAuth token has become invalid. This can happen due to Airtable API issues or if the token was revoked. You have two options to fix this:

**Option 1: Re-authenticate with OAuth**
...
- A new tab/window will open - select **All current and future bases in all current and future workspaces**.
- Click **Grant access**.

**Option 2: Switch to Personal Access Token (PAT) - Recommended**

Personal Access Tokens are more reliable and don't expire unexpectedly. To switch:

- Go to [Airtable's token creation page](https://airtable.com/create/tokens/new).
- Create a new token with access to all bases and these scopes: data.records:read, data.records:write, schema.bases:read, webhook:manage.
- Hover over the Data Fetcher extension, then click the **settings icon** (⚙️).
- Under **Workspaces**, click **Manage workspace**.
- Under **Airtable Personal Access Token (PAT)**, click **+ Add** and paste your token.

For detailed instructions, see [Authorize with a Personal Access Token (PAT)](https://help.datafetcher.com/account/authorize-data-fetcher-airtable#authorize-with-a-personal-access-token-pat).
