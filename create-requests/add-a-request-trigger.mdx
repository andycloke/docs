...
Once your trigger is active, you can verify it's working by making a test change to your table and checking the [Run History](https://help.datafetcher.com/organise-requests/run-history) to see if the request executed automatically.
...
<Warning>
  Enabling API change detection can cause infinite loops. This happens when a trigger-created or trigger-updated record causes the same trigger to run again, potentially consuming your entire monthly usage limit. Only enable this setting if you're certain your workflow won't create feedback loops.
</Warning>