...
This setting determines when Data Fetcher pauses your automation (schedules, webhook URLs, and triggers) to prevent runaway failures.
...
When enabled, a run that returns zero records after transformation and mapping is treated as an error — even if the API call itself succeeded. This is useful when you expect data to always be present (e.g. daily sales, inventory updates) and want to be alerted when it's missing.

Leave this off if data volume naturally varies or empty periods are normal for your use case.

Empty results count toward your consecutive error thresholds.
...
You're running a request for each of 100 customer IDs from your Airtable, calling an API to get order details for each customer.
