...
When you run a request, the response data gets transformed into a tabular format with fields and records for your destination table. You control this process using response field mapping and response data transformation settings.

## **Understanding the two transformation types**

Data Fetcher offers two ways to handle nested arrays in your API responses: **Single record per item** and **Expand nested items**. The key difference is how they treat nested data.
...
<Info>
  **Single record per item** doesn't mean everything goes into one record. Data
  Fetcher finds the first array in your response and creates one record for each
  item in that array.
</Info>

### **Expand nested items**

Expand nested items creates separate records for each nested item, duplicating parent data across multiple rows:
